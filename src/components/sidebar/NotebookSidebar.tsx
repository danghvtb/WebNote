...
import { Plus, MoreVertical, Notebook, FileText, ClipboardList, ChevronRight, ChevronDown, Pencil, Copy, Trash2, AlertTriangle } from 'lucide-react';
    ...
    <aside className="w-full h-full flex flex-col overflow-hidden glass-sidebar">
      ...
      <div className="flex items-center justify-between px-4 py-3" style={{ borderBottom: '1px solid var(--color-border)' }}>
        <span className="text-sm font-medium" style={{ color: 'var(--color-text-primary)' }}>
          {selectedDayDate ? formatDateFull(selectedDayDate) : 'Notebooks'}
        </span>
        <div className="relative">
          <button onClick={() => setPlusMenuOpen((open) => !open)} className="p-1.5 rounded-lg transition-colors cursor-pointer" style={{ color: 'var(--color-accent)' }} title="Thêm sổ ghi chú hoặc báo cáo" aria-label="Thêm sổ ghi chú hoặc báo cáo"><Plus className="w-4 h-4" /></button>
